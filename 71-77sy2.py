...
def private_key_to_wif(private_key: int) -> str:
    ...
    extended_key = b'\x80' + priv_key_bytes + b'\x01'
...
def save_results(results, filename: str = "found_addresses.json"):
    """保存匹配结果到JSON文件"""
...
def clear_previous_files():
    """清空之前的记录文件"""
    files_to_clear = ["all_generated_keys.csv", "interrupted_results.json", "error_results.json"]
    for filename in files_to_clear:
        if os.path.exists(filename):
            try:
                ...
                print(f"清空文件 {filename} 时出错: {e}")

def worker_process(process_id, target_addresses, shared_results, total_attempts, found_counter, found_lock):
    """工作进程函数"""
    print(f"进程 {process_id} 启动")
    local_attempts = 0
    local_start_time = time.time()
    
    while found_counter.value < len(target_addresses):
        local_attempts += 1
        with total_attempts.get_lock():
            total_attempts.value += 1
        
        # 生成随机私钥和地址
        private_key_int = generate_private_key()
        ...
        if address in target_addresses:
            ...
            # 如果找到所有目标地址，提前结束
            if found_counter.value >= len(target_addresses):
                break
        ...
        if local_attempts % 10000 == 0:
            ...
            print(f"进程 {process_id}: 已尝试 {local_attempts:,} 次, 速度: {rate:.1f} 次/秒")
    
    # 进程完成统计
    elapsed_time = time.time() - local_start_time
    print(f"进程 {process_id} 完成: 尝试 {local_attempts:,} 次, 平均速度: {local_attempts/elapsed_time:.1f} 次/秒")

def generate_and_search_multiprocess(num_processes: int = 20):
    """多进程生成私钥并搜索目标地址"""
    target_addresses = {
...
def main():
    ...
    print("完全随机生成私钥，搜索整个私钥空间")
    print("=" * 50)
    
    # 配置参数
    num_processes = 120  # 进程数量
    
    try:
        results = generate_and_search_multiprocess(num_processes=num_processes)
