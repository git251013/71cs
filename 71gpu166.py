...
"""
比特币私钥扫描器 - 高效GPU加速版本
扫描范围: 2243124079363081476505 到 2343124079363081476505
...
class GPUWorker:
    ...
    def process_batch(self, key_batch):
        ...
        # 预计算目标地址的hash160，避免重复计算
        target_hash160 = self.address_to_hash160(self.target_address)
        ...
        while True:
            ...
            # 使用更快的地址生成和比较方法
            address = self.private_key_to_compressed_address_fast(key, target_hash160)
            batch_processed += 1
    ...
    def private_key_to_compressed_address_fast(self, private_key_int, target_hash160):
        """更快的私钥到地址转换 - 直接比较hash160"""
        try:
            # 计算公钥点
            public_point = point_multiply(private_key_int, (self.ec_params.Gx, self.ec_params.Gy), 
                                         self.ec_params.p, self.ec_params.a)
...
class ProcessWorker:
    ...
    def __init__(self, process_id, start_key, end_key, batch_size, target_address, result_queue, progress_queue, stop_event):
        ...
        # 创建线程池 - 根据CPU核心数优化
        self.thread_pool = ThreadPoolExecutor(max_workers=8)  # 增加线程数
        self.gpu_workers = [GPUWorker(i, self.ec_params, target_address) for i in range(8)]
    ...
    def run(self):
        ...
        while current_key < self.end_key and not self.stop_event.is_set():
            # 计算当前批次大小 - 使用更大的批次
            actual_batch_size = min(self.batch_size * 10, self.end_key - current_key)  # 增加批次大小
            ...
            if current_time - self.last_report_time >= 10:  # 每10秒报告一次
                ...
                self.progress_queue.put((self.process_id, self.processed, progress, current_key))
...
class BitcoinKeyScanner:
    def __init__(self, num_processes=None, batch_size=100000):
        ...
        self.end_range = 2343124079363081476505
        self.batch_size = batch_size
        self.target_address = TARGET_ADDRESS
        
        # 根据系统资源自动配置 - 优化配置
        if num_processes is None:
            # 使用80%的CPU核心，充分利用20核CPU
            self.num_processes = max(1, int(mp.cpu_count() * 0.8))
        else:
            self.num_processes = num_processes
            
        # 限制最大进程数
        self.num_processes = min(self.num_processes, 20)  # 最大20个进程
        ...
        print(f"扫描范围: {self.start_range} 到 {self.end_range}")
    ...
    def stats_reporter(self):
        ...
        while not self.stop_event.is_set():
            ...
            while not self.progress_queue.empty():
                try:
                    process_id, processed, progress, current_key = self.progress_queue.get_nowait()
                    process_stats[process_id] = (processed, progress, current_key)
                except queue.Empty:
                    break
            ...
            if process_stats:
                print(f"  进程状态:")
                for pid, (proc_processed, proc_progress, current_key) in process_stats.items():
                    print(f"    进程 {pid}: {proc_progress:.6f}% ({proc_processed} 密钥) - 当前: {current_key}")
